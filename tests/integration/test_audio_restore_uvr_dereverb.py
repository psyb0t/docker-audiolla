"""End-to-end test for ``POST /v1/audio/restore/uvr-dereverb``.

UVR BS-Roformer de-reverberation. Weights download from HuggingFace on
first use (≈500 MB pull, licence-gated). Marked ``gpu`` + ``hf_gated``.
"""

from __future__ import annotations

import httpx
import pytest

from .helpers import assert_wav, uvr_model_produced_no_output

pytestmark = [
    pytest.mark.engine("uvr-dereverb"),
    pytest.mark.gpu,
    pytest.mark.hf_gated,
]


def test_restore_writes_real_audio(
    client: httpx.Client, staged_long_audio: str,
) -> None:
    """Happy path: 200 + the staged output is a decodable WAV."""
    r = client.post(
        "/v1/audio/restore/uvr-dereverb",
        json={
            "file_path": staged_long_audio,
            "output_path": "out/dereverb.wav",
        },
        timeout=900.0,
    )
    if uvr_model_produced_no_output(r):
        return  # synthetic sine fixture has nothing for UVR to extract
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["path"] == "out/dereverb.wav"
    assert body["engine"] == "uvr-dereverb"
    assert body["output_format"] == "wav"

    fetched = client.get(f"/v1/files/{body['path']}")
    assert fetched.status_code == 200
    assert_wav(fetched.content, min_bytes=10_000)


def test_restore_rejects_wrong_engine_type(
    client: httpx.Client, staged_long_audio: str,
) -> None:
    """A registered engine that doesn't expose ``.restore()`` → 400 or 404."""
    r = client.post(
        "/v1/audio/restore/ffmpeg-render",
        json={
            "file_path": staged_long_audio,
            "output_path": "out/x.wav",
        },
    )
    assert r.status_code in (400, 404), r.text


def test_restore_requires_input(client: httpx.Client) -> None:
    """Missing both file_path and file_url → 400 (xor validator)."""
    r = client.post(
        "/v1/audio/restore/uvr-dereverb",
        json={"output_path": "out/x.wav"},
    )
    assert r.status_code == 400, r.text


def test_restore_requires_output(
    client: httpx.Client, staged_long_audio: str,
) -> None:
    """Missing both output_path and output_url → 400 (xor validator)."""
    r = client.post(
        "/v1/audio/restore/uvr-dereverb",
        json={"file_path": staged_long_audio},
    )
    assert r.status_code == 400, r.text
