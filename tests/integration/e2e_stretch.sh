...
test_stretch_identity() {
    ...
    head -c 4 "$tmp" | grep -q "RIFF" || { echo "  FAIL: not WAV"; rm -f "$tmp"; return 1; }
    rm -f "$tmp"
...
" 2>/dev/null || echo "0")
    ...
    local tmp code
    tmp=$(mktemp --suffix=.wav)
    code=$(curl -s -o "$tmp" -w "%{http_code}" --max-time 90 \
        -X POST -F "file=@${FIXTURE}" \
        -F "tempo_factor=0.5" \
        "${AUDIOLLA_BASE_URL}/v1/audio/stretch")
...
test_stretch_output_format_mp3() {
    local ct
    ct=$(curl -s -o /dev/null -w "%{content_type}" --max-time 60 \
        -X POST -F "file=@${FIXTURE}" \
        -F "output_format=mp3" \
        "${AUDIOLLA_BASE_URL}/v1/audio/stretch")
    if ! echo "$ct" | grep -qi "mpeg\|mp3"; then
        echo "  FAIL: content-type not mp3: $ct"; return 1
    fi
...
test_stretch_output_path() {
    ...
    body=$(curl -s --max-time 90 -X POST \
        -F "file=@${FIXTURE}" \
        -F "tempo_factor=1.25" \
        -F "output_path=stretch/fast.wav" \
        "${AUDIOLLA_BASE_URL}/v1/audio/stretch")
